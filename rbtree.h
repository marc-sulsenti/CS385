...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        //Following psuedo-code implementation from CLRS pg 315.
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        ...
        //If there is an RBT do the following..
        if(n!= nullptr){
            delete_tree(n->left);
            delete_tree(n->right);
            delete n;
    ...
    void insert_fixup(Node<K, V> *z) {
        ...
        //Following psuedo-code implementations from the textbook
    ...
    void left_rotate(Node<K, V> *x) {
        // TODO
        //Following psuedo-code implementations from textbook
        Node<K,V> *y = x->right;
    ...
    void right_rotate(Node<K, V> *x) {
        ...
        //Following psuedo-code implementations  from textbook, inferred from left_rotate (symmetric from left_rotate)
    ...
    int height(Node<K, V> *node) const {
        ...
        //Special case from instructions (-1)
        //Height for an empty tree shold be -1
        ...
        //Determining which of the subtrees has the greater height
    ...
    size_t leaf_count(Node<K, V> *node) const {
        ...
        //Itterate through the RBT to find every leaf node and add one when its found.

        //Case to go left
        if(node->left != nullptr){
            leaf_counter = leaf_counter + leaf_count(node->left);
        }

        //Case to go right
        if(node->right != nullptr){
                leaf_counter = leaf_counter + leaf_count(node->right);
        }

        //Case where you reach a leaf node
    ...
    int diameter(Node<K, V> *node) const {
        ...
        diameter_left = diameter(node->left);
        diameter_right = diameter(node->right);

        //Finding the proper diameter of the RBT

        if(diameter_left < diameter_right){
            diameter_left = diameter_right;
        }

        path_sum = path_left+path_right;

        if(path_sum > diameter_left){
            return path_sum;
        }

        return diameter_left;
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        ...
        //Case for if there is a nullptr at the specified node
        if(node== nullptr) return 0;

        //At the 0th level, (root) the width will be one
        if(level == 0 ) return 1;


        width_left = width(node->left, level-1);
        width_right = width(node->right, level -1);
    ...
    size_t null_count(Node<K, V> *node) const {
        ...
        //Handling of the base case where node is null, (1)
        if(node == nullptr) return 1;

        //itterate recursively through the RBT
        count_left = null_count(node->left);
        count_right = null_count(node->right);
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        ...
        //Handling of the base case
        if( node == nullptr){
            return 0;
        }

        //Recursive itteration through the tree
        level_left =  sum_levels(node->left,level+1);
        level_right = sum_levels(node->right,level+1);

        return level+level_left+level_right;
    }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        ...
        //Recursive itteration through the tree
        level_left =  sum_null_levels(node->left,level+1);
        level_right = sum_null_levels(node->right,level+1);
